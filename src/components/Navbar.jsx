                ...
                <div className="flex items-center justify-between h-16 lg:h-20">
                    ...
                    <nav className="hidden lg:flex items-center gap-1">
                        {navItems.map(item => (
                            <Link key={item.to} to={item.to}
                                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                                    isActive(item.to) ? 'bg-[#174A43] text-white shadow-xs' : 'text-[var(--foreground-secondary)] hover:text-[var(--foreground)] hover:bg-[var(--muted-bg)]'
                                }`}>
                                {item.label}
                            </Link>
                        ))}
                        {user && (
                            <Link to="/dashboard"
                                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                                    isActive('/dashboard') ? 'bg-[#174A43] text-white shadow-xs' : 'text-[var(--foreground-secondary)] hover:text-[var(--foreground)] hover:bg-[var(--muted-bg)]'
                                }`}>
                                Dashboard
                            </Link>
                        )}
                        ...
                        {user ? (
                            <div className="relative">
                                <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                                    ...
                                    <div className="w-8 h-8 rounded-lg bg-[#174A43] flex items-center justify-center text-white text-xs font-bold">
                            ...
                            {user ? (
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 p-4 rounded-xl bg-[var(--muted-bg)]">
                                        <div className="w-10 h-10 rounded-lg bg-[#174A43] flex items-center justify-center text-white font-bold">
...
function MobileNavItem({ to, label, icon: Icon, active }) {
    return (
        <Link to={to} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${
            active ? 'bg-[#174A43] text-white' : 'text-[var(--foreground-secondary)] hover:text-[var(--foreground)] hover:bg-[var(--muted-bg)]'
        }`}>
            <Icon size={18} /> {label}
        </Link>
    );
