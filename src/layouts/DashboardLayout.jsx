                        ...
                        {sidebarItems.map(item => (
                            <Link key={item.to} to={item.to}
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group relative ${
                                    isActive(item.to) ? 'bg-black text-white shadow-sm font-semibold' : 'text-[var(--foreground-secondary)] hover:text-black hover:bg-[var(--muted-bg)]'
                                }`}
                    ...
                    <div className={`p-3 border-t border-[var(--card-border)] ${!sidebarOpen && !mobileOpen ? 'flex justify-center' : ''}`}>
                        {(sidebarOpen || mobileOpen) ? (
                            <div className="space-y-1">
                                ...
                                <button onClick={logout} className="flex items-center gap-3 w-full px-3 py-2 rounded-xl text-xs font-medium text-red-600 hover:bg-red-50 transition-all"><LogOut size={16} /> Sign Out</button>
                            </div>
                        ) : (
                            <button onClick={logout} className="p-2 rounded-xl text-red-600 hover:bg-red-50" title="Sign Out"><LogOut size={18} /></button>
                        )}
                    </div>
                </div>
            </aside>
